...
export default defineConfig(({ mode }) => {
  ...
  // The API reads its secrets from process.env, the same as on Vercel. In
  // development they come from .env files, which Vite loads but does not
  // otherwise expose to Node.
...
/** Hop-by-hop headers a Request must not carry, plus ones its body re-derives. */
...
async function toWebRequest(req: IncomingMessage): Promise<Request> {
  ...
  for (const [key, value] of Object.entries(req.headers)) {
    if (SKIPPED_REQUEST_HEADERS.has(key)) continue
  ...
  if (method !== "GET" && method !== "HEAD") {
    ...
    // Copied into a standalone Uint8Array: Request will not take Node's Buffer,
    // whose views share a pooled backing store. Buffering is fine here because
    // media bytes go straight to Vercel Blob and never reach this middleware.
...
/**
 * Serves the Vercel Function's Hono app as dev middleware so `/api/*` answers
 * the same way locally as it does in production, without a second process.
 * `ssrLoadModule` transpiles server/ on demand, so edits there take effect on
 * the next request rather than needing a restart.
 */
function apiDevServer(): Plugin {
  return {
    name: "potret-api-dev-server",
    apply: "serve",
        ...
        void (async () => {
          try {
            ...
            await writeWebResponse(
              res,
              await app.fetch(await toWebRequest(req)),
            )
          } catch (err) {
